...
{
    /// <summary>
    /// A Interaction to judge if uppercut action is performed
    /// depending on the DGXRController.uppercut control value.
    /// </summary>
    public class UppercutInteraction : MetaverseInteraction, IInputInteraction
    {
        public void Process(ref InputInteractionContext context)
        {
            if (context.control.device is DGXRHumanController dgXRDevice)
            {
                DGXR.Logger.Log("------------------Action 调试信息：Uppercut = " + dgXRDevice.Uppercut.ReadValue() + " " + context.phase);
        ...
        {
            return dgXRDevice.Uppercut.ReadValue() > Confidence;
        }
