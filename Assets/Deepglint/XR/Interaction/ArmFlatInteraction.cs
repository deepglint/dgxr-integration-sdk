...
{
    /// <summary>
    /// A Interaction to judge if armFlat action is performed
    /// depending on the DGXRController.armFlat control value.
    /// </summary>
    public class ArmFlatInteraction : MetaverseInteraction, IInputInteraction
    {
        public void Process(ref InputInteractionContext context)
        {
            if (context.control.device is DGXRHumanController dgXRDevice)
            {
                DGXR.Logger.Log("------------------Action 调试信息：ArmFlat = " + dgXRDevice.ArmFlat.ReadValue() + " " + context.phase);
        ...
        {
            return dgXRDevice.ArmFlat.ReadValue() > Confidence;
        }
