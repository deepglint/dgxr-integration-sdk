...
namespace Runtime.Samples.CustomPlayerManager
{
    public class Options
    {
        ...
        public void OnMessage(string res)
        {
            ...
            if (res != null)
            {
                ...
                {
                    Debug.Log("1. WS 连接 - 1.3 骨骼人数：" + info.Pose.Count);
                }
    ...
    public class Source : MonoBehaviour
    {
        ...
        public void Init(Options arg)
        {
            ...
            Debug.Log("1. WS 连接 - 1.1 地址：: " + wsUri);
            Connect(wsUri);
            ...
            if (_autoReconnect && !this.hasConnectSuccess)
            {
                ...
                if ((GetNowTime() - _lastConnect) * 1000 > maxWait)
                {
                    if (_reconnectCount < _reconnectMaxCount || _reconnectMaxCount == -1)
                    {
                        ...
                        Debug.Log("重连次数:" + _reconnectCount);
                        Connect(wsUri);
                    }
                    else
                    {
                        ...
                        Debug.Log("re-reconnect: " + _reconnectCount + "次，重连次数过多，不再继续重连，请联系后端服务人员处理");
                    }
                }
            }
        }
        ...
        {
            Debug.Log("1. WS 连接 - 1.2.1 连接成功 server：web socket open!");
            _lastConnect = GetNowTime();
        ...
        {
            Debug.LogError("1. WS 连接 - 1.2.2 连接失败 " + error);
            hasConnectSuccess = false;
        ...
        {
            Debug.Log("server warning: web socket closed!");
        }
