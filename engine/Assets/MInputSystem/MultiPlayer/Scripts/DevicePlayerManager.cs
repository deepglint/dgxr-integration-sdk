...
namespace Moat
{
    ...
    public class DevicePlayerManager: MonoBehaviour
    {
        ...
        //链接服务器成功回调
        private Action OnOpenWssAction;
        //断开服务器回调
        private Action OnCloseWssAction;

        [HideInInspector]private int playerCount = 1;
        
        public GameObject PlayerPrefab;
        [HideInInspector]public bool IsGlobalTest = false;
        [HideInInspector]private int gamepadCount = 0;
        //后端元数据
        ...
        void Start()
        {
            ...
            gamepadCount = DisplayData.configDisplay.gamepad;

            if (DebugCanvas != null)
            {
                DebugCanvas.targetDisplay = DisplayData.configDisplay.targetDisplay.debug - 1;
                DebugCanvas.gameObject.SetActive(IsGlobalTest);
            }
        ...
        private void Update()
        {
            ...
            foreach (KeyValuePair<string, BodyDataSource> person in personBodySource)
            {
                ...
                // MDebug.Log("personInfo.angle::"+personInfo.angle);
                //MDebug.Log("FootPos::"+FootPos.x +"-"+FootPos.y);
        ...
        private void DebugUpdate()
        {
            ...
            if (IsGlobalTest)
            {
                ...
                if (verticalInput != 0 || horizontalInput != 0)
                {
                    ...
                    // MDebug.Log(vector2.x + " :: "+vector2.y);
